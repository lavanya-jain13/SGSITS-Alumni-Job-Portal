...
exports.postJob = async (req, res) => {
  try {
    ...
    // Resolve company: prefer provided company_id if it belongs to this alumni, else fallback to first
    let company = null;
    if (req.body.company_id) {
      company = await db("companies")
        .where({ id: req.body.company_id, alumni_id: alumniProfile.id })
        .first();
    }
  ...
  try {
    ...
    const applicants = await db("job_applications as ja")
      ...
      .select(
        ...
        "sp.skills as student_skills",       // <-- merged from first file
